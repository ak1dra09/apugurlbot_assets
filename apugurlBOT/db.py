...
class Database:
    ...
    def finish_drop_random(self, drop_id: int, chat_id: int) -> tuple[list[tuple[int, str | None, str | None, int]], int]:
        with self.transaction() as connection:
            drop = connection.execute("SELECT * FROM drops WHERE id = ?", (drop_id,)).fetchone()
            if not drop:
                raise ValueError("drop_not_found")
            ...
            winners = connection.execute(
                """
                SELECT tg_id, username, full_name FROM chat_members
                WHERE chat_id = ? AND tg_id != ? AND tg_id != 777000 AND is_bot = 0 ORDER BY RANDOM() LIMIT ?
                """,
                (chat_id, drop["creator_tg_id"], drop["winners_limit"]),
            ).fetchall()
            if not winners:
                connection.execute("UPDATE drops SET closed = 1 WHERE id = ?", (drop_id,))
                connection.execute("UPDATE users SET balance = balance + ? WHERE tg_id = ?", (drop["total_amount"], drop["creator_tg_id"]))
                return [], 0
            base_amount, remainder = divmod(drop["total_amount"], len(winners))
            connection.execute("UPDATE drops SET closed = 1 WHERE id = ?", (drop_id,))
            payouts = []
            for index, winner in enumerate(winners):
                ...
                connection.execute(
                    "INSERT INTO drop_claims(drop_id, tg_id) VALUES (?, ?)",
                    (drop_id, winner["tg_id"]),
                )
                connection.execute(
                    "UPDATE users SET balance = balance + ? WHERE tg_id = ?",
                    (payout, winner["tg_id"]),
                )
                payouts.append((winner["tg_id"], winner["username"], winner["full_name"], payout))
