...
class Database:
    ...
    def init_schema(self) -> None:
        self.connection.executescript(
            ...
            CREATE TABLE IF NOT EXISTS burn_cursor (
                id INTEGER PRIMARY KEY CHECK (id = 1),
                last_lt INTEGER NOT NULL DEFAULT 0
            );
    ...
    def get_burn_cursor(self) -> int:
        row = self.connection.execute("SELECT last_lt FROM burn_cursor WHERE id = 1").fetchone()
        return row["last_lt"] if row else 0

    def set_burn_cursor(self, last_lt: int) -> None:
        self.connection.execute(
            """
            INSERT INTO burn_cursor(id, last_lt) VALUES (1, ?)
            ON CONFLICT(id) DO UPDATE SET last_lt = excluded.last_lt
            """,
            (last_lt,),
        )
    ...
    def record_burn_event(self, event_id: str, raw_amount: int, sender_address: str | None) -> bool:
        with self.transaction() as connection:
            if connection.execute("SELECT 1 FROM burn_events WHERE event_id = ?", (event_id,)).fetchone():
                return False
            connection.execute(
                "INSERT INTO burn_events(event_id, raw_amount, sender_address) VALUES (?, ?, ?)",
                (event_id, str(raw_amount), sender_address),
            )
            return True

    def get_total_burned_raw(self) -> int:
        rows = self.connection.execute("SELECT raw_amount FROM burn_events").fetchall()
        return sum(int(row["raw_amount"]) for row in rows)
